...
const{getSalaryInfo,getEmployeeCompleteInformation,getUserprofileDetail}=require("./employeeFunction")
const{getGenericMonthSummary,getUserMonthAttendaceComplete,getUserMonthPunching,
    getUserMonthLeaves,_getPreviousMonth,_getDatesBetweenTwoDates,getDaysOfMonth}=require("./leavesFunctions");
const db = require("./db");
...
let getSalaryBlocks=async(userid, year, month, extra_arrear, arrear_for_month, checkPreviousMonthPayslip,db,req)=>{
   ...
   if( DEBUG ){
    ...
    let MONTH_TOTAL_WORKING_DAYS = await getTotalWorkingDays(year, month, userid,db);

    /* emp working dayss without punch time */
    let empWithoutPunchDaysData = await getEmployeeWorkingDaysWithoutPunchTime(userid, year, month,db,req,arrear_for_month);
    let employeeWorkingDaysWithoutPunchTime = empWithoutPunchDaysData['pending_leaves'];
    let emp_toshow_pending_leaves = empWithoutPunchDaysData['toshow_pending_leaves'];
    let emp_toshow_days_before_joining = empWithoutPunchDaysData['toshow_days_before_joining'];
    let emp_applied_pending_leaves = empWithoutPunchDaysData['applied_pending_leaves'];
        ...
        if( employeePreviousPayslips.length > 0 ){
            ...
            let current_month_previous_month_year = await _getPreviousMonth( currentYear, currentMonth);
            let current_month_previous_year = current_month_previous_month_year['year'];
            let current_month_previous_month = current_month_previous_month_year['month'];
        ...
        if (c.length > 0) {
            for (let v of c ) {
                ...
                if ( vStatus == "approved" || vStatus == "pending" ) {
                    if( vStatus == "approved" && v['leave_type'] && (v['leave_type'].toLowerCase() == 'rh compensation' || (v['leave_type']).toLowerCase() == 'restricted') ){
                        ...
                        let daysOfLeaves = await _getDatesBetweenTwoDates(v['from_date'], v['to_date']);                        
                                ...
                                for (let [key,dl] of Object.entries(daysOfLeaves)) {                                
                                    if( ( new Date(dl).getMonth()+1) == month ){                                        
                                        if( !(dl in current_month_leave_days) ){                                            
                                            ...
                                            user_salaryinfo['days_present'] = user_salaryinfo['days_present'] - 1;
                                        }                                    
                                    }                                
                                }
                            } 
        ...
        if( daysPresent.length > 0 ){
         ...
         if( Array.isArray(salaries) && salaries.length > 0 ){
            ...
            for (let [key,salary] of Object.entries(salaries)) {
               ...
               let daysBWfromAndTo = await _getDatesBetweenTwoDates(salaryApplicableFromDate,salaryApplicableTillDate);
...
if( (rawBlocks.length) > 0 ){
    ...
    for(let [key,value] of Object.entries(tempRawBlock)) {
        ...
        let block_days_without_punch = array_values(_.intersection(genericDays, emp_toshow_pending_leaves));
        ...
        block_leaves_allocated = ( month_leaves_allocated / MONTH_TOTAL_WORKING_DAYS ) * block_total_working_days;
        block_leaves_allocated = Math.round( block_leaves_allocated, 2 );
        /*calculate block balance leave*/
        block_balance_leave = ( balance_leave / MONTH_TOTAL_WORKING_DAYS ) * block_total_working_days;
        ...
        if( ( temp_current_month_leave_days.length) > 0 ){
            for (let [key,value] of Object.entries(workingDays)) {
                if( (value['full_date'] in  current_month_leave_days) ){
                    ...
                    block_current_month_leave += current_month_leave_days[value['full_date']];
                }
            }
        }
        /*get final leave balance of employee*/
        leaves = block_balance_leave - block_current_month_leave + block_leaves_allocated;
        unpaid_leave = 0;
        if (leaves >= 0) {
        ...
        actual_salary_detail = salary_detail =await getSalaryDetail(user_salaryinfo['id'],userid=false,db);
        ...
        final_basic = (block_total_working_days * pday_basic)  - ( pday_basic * unpaid_leave);
        final_hra = (block_total_working_days * pday_hra)  - ( pday_hra * unpaid_leave);
        final_conve = (block_total_working_days * pday_conve)  - ( pday_conve * unpaid_leave);
        ...
        final_arrear = (block_total_working_days * pday_arrear)  - ( pday_arrear * unpaid_leave);
        ...
        */
        ...
        salary_detail['Conveyance'] = final_conve*1 > 0 ? Math.round(final_conve, 2) : 0;
        salary_detail['Medical_Allowance'] = final_med*1 > 0 ? Math.round(final_med, 2) : 0;
        salary_detail['Special_Allowance'] = final_spl*1 > 0 ? Math.round(final_spl, 2) : 0;
        salary_detail['Arrears'] = final_arrear*1 > 0 ? Math.round(final_arrear, 2) : 0;

        salary_detail['TDS'] = final_tds * 1 > 0 ? Math.round(final_tds, 2) : 0;
        salary_detail['Misc_Deductions'] = final_misc * 1 > 0 ? Math.round(final_misc, 2) : 0;
        salary_detail['Advance'] = final_advance * 1 > 0 ? Math.round(final_advance, 2) : 0;
        salary_detail['Loan'] = final_loan * 1 > 0 ? Math.round(final_loan, 2) : 0;
        ...
        final_leave_balance = block_balance_leave + block_leaves_allocated - block_current_month_leave;
...
if( (blocks.length) > 0 ){
   ...
   bs_bonus = 0;
    ...
    if (!_.isEmpty(extra_arrear) && !_.isEmpty(arrear_for_month)) {
       ...
       final_arrear = await checkArrearDetail(userid, year, month, extra_arrear, arrear_for_month,db);
    ...
    if( checkPreviousMonthPayslip ){
        checkEmployeeLastMonthPayslipData =await checkEmployeeLastMonthPayslipStatus(userid, year, month,db);
        if( checkEmployeeLastMonthPayslipData != false ){
            r_data['last_month_payslip_pending'] = checkEmployeeLastMonthPayslipData;
            net_salary_including_previous_month =  user_data_for_payslip['net_salary'] * 1 + checkEmployeeLastMonthPayslipData['user_data_for_payslip']['net_salary'] * 1;
            user_data_for_payslip['net_salary_including_previous_month'] = Math.round(net_salary_including_previous_month, 2);
        }
    }
    /* check if last month payslip is genereated or not */
    finalArray['employee_toshow_pending_leaves'] = emp_toshow_pending_leaves;
    finalArray['employee_toshow_days_before_joining'] = emp_toshow_days_before_joining;
    ...
    finalArray['all_users_latest_payslip'] =await getAllUserPayslip(userid, year, month,db);
...
let checkEmployeeLastMonthPayslipStatus=async(userid, year, month,db)=>{
    ...
    if(isEligibleForLastMonthSalary){
        ...
        for (let [key,ps] of Object.entries(empPreviousPayslips)) {
            if( ps['year'] * 1 == toCheckYear * 1 && ps['month'] * 1 == toCheckMonth * 1 ){
                lastMonthPayslipGenerated = true;
            }
        }
        if( lastMonthPayslipGenerated == false ){
            let lastMonthPayslip =await getUserManagePayslip( userid, toCheckYear, toCheckMonth, 0, 0, false,db);
...
let getUserManagePayslip=async(userid, year, month, extra_arrear, arrear_for_month, checkPreviousMonthPayslip = true,db)=>{ $r_error = 1;
    let r_message = "";
    let r_data = {};
    let date = year + "-" + month + "-01";
    let month_name = date('F', strtotime(date)); new Date(date).toLocaleString('default', { month: 'long' });
    let user_salaryinfo =[];
    ...
    user_salaryinfo['total_working_days'] = await getTotalWorkingDays(year, month, userid,db);
    //get employee month attendance
    let dayp = await getUserMonthPunching(userid, year, month,db);

    /* emp working dayss without punch time */
    let empWithoutPunchDaysData = await getEmployeeWorkingDaysWithoutPunchTime(userid,year,month,db);
    let employeeWorkingDaysWithoutPunchTime = empWithoutPunchDaysData['pending_leaves'];
    let emp_toshow_pending_leaves = empWithoutPunchDaysData['toshow_pending_leaves'];
    let emp_toshow_days_before_joining = empWithoutPunchDaysData['toshow_days_before_joining'];
    let emp_applied_pending_leaves = empWithoutPunchDaysData['applied_pending_leaves'];
    ...
    //get employee month salary details
    let actual_salary_detail = salary_detail = await getSalaryDetail(user_salaryinfo['id'], userid);
    //get misc deduction of salary month form payslips table. 
   ...
   res = await getUserPayslipInfo(userid,db);
    ...
    if ((c.length) > 0) {
        for(let v of c ) {
            let vStatus = (v['status']).toLowerCase();
            if ( vStatus == "approved" || vStatus == "pending" ) {
                if( vStatus == "approved" && _.isSet(v['leave_type']) && (v['leave_type'].toLowerCase()) == 'rh compensation' ){
    ...
    // get final leave balance of employee
    let leaves = balance_leave - current_month_leave + user_salaryinfo['leaves_allocated'];
    if (leaves >= 0) {
    ...
    if( checkPreviousMonthPayslip ){
        ...
        if( checkEmployeeLastMonthPayslipData != false ){
            r_data['last_month_payslip_pending'] = checkEmployeeLastMonthPayslipData;
            net_salary_including_previous_month =  user_salaryinfo['net_salary'] * 1 + checkEmployeeLastMonthPayslipData['user_data_for_payslip']['net_salary'] * 1;
            user_salaryinfo['net_salary_including_previous_month'] = Math.round(net_salary_including_previous_month, 2);
        }
    }
    /* check if last month payslip is genereated or not */
    ...
    if( _.isSet(user_salaryinfo['total_salary']) && _isSet(user_salaryinfo['total_earning']) ){
        total_leaves_amount_deducted = user_salaryinfo['total_salary'] * 1 - user_salaryinfo['total_earning'] * 1;
    }
    user_salaryinfo['total_leaves_amount_deducted'] = Math.round(total_leaves_amount_deducted, 2);
    ...
    r_data['employee_toshow_days_before_joining'] = emp_toshow_days_before_joining;
    ...
    r_data['all_users_latest_payslip'] =await getAllUserPayslip(userid, year, month,db);
...
let getAllUserPayslip=async(userid, year, month,db)=>{
    let row =await db.sequelize.query(`select * from payslips where month='${month}' AND year = '${year}'`,{type:QueryTypes.SELECT});
    return row;
}
let getSalaryDetail=async(salary_id,userid=false,db)=>{
    let ret = [];
    let row =await db.sequelize.query(`select * from salary_details where salary_id = '${salary_id}'`,{type:QueryTypes.SELECT});
    for (let val of row) {
    ...
    if( userid != false ){
        ...
        if(Array.isArray(holdingDetails) && (holdingDetails.length) > 0 ){
            ...
            if( _.isSet(holdingInfo['holding_amt']) && holdingInfo['holding_amt'] * 1 > 0 ){
      ...
      /* added by arun on 9th oct 2019 for holiding amount to be calculate in API*/
        ...
        sum_earnings = ret['Basic'] + ret['HRA'] + ret['Conveyance'] + ret['Medical_Allowance'] + ret['Special_Allowance'];
...
let checkArrearDetail=async(userid, year, month, extra_arrear, arrear_for_month,db)=>{
...
let getUserBalanceLaveInfo=async(userid,year,month,db)=>{
    ...
    let q = await db.sequelize.query(`select * from payslip where user_Id = '${userid}' ORDER by id DESC`,{type:QueryTypes.SELECT});
    let r = {};
    for (let val of row ) {
...
let  getUserPayslipInfo=async(userid,db,hidePayslip = false)=>{
    let row =await db.sequelize.query(`select * from payslips where user_Id = '${userid}' ORDER by id DESC`,{type:QueryTypes.SELECT});
    if( hidePayslip == true ){
        if( sizeof(row) > 0 ){
...
let getUserMiscDeduction=async(userid, year, month,db)=>{
    let result = 0;
    let q =await db.sequelize.query(`SELECT * FROM payslips WHERE user_Id= '${userid}'  AND month = '${month}' AND year= '${year}'`,{type:QueryTypes.SELECT});
    if (q.length > 0) {
        result = q[0]['misc_deduction_2'];
    }
    return result;
}
let getEmployeeWorkingDaysWithoutPunchTime=async(userid,year,month,db,req,arrear_for_month)=>{
                ...
                if( day['day_type'] == 'WORKING_DAY' ){
                   let dayDate = day['full_date'];
                    ...
                    if( dayInTime == "" && dayOutTime == "" ){
                        ...
                        if( new Date(dayDate).getTime() >= new Date(emp_dateofjoining).getTime() ){
                            return_toshow_pending_leaves.push(dayDate);
                        } else {
                            return_toshow_days_before_joining.push(dayDate);
                        }
                    }
                } else if( day['leave_status'] && (day['leave_status']).toLowerCase() == 'pending' ){
...
module.exports={
    deleteUserSalary,getUserManagePayslipBlockWise
}